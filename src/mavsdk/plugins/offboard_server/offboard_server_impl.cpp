...
OffboardServerImpl::OffboardServerImpl(std::shared_ptr<ServerComponent> server_component) : ServerPluginImplBase(server_component)
{
...
void OffboardServerImpl::subscribe_position_target_global_setpoint(OffboardServer::PositionTargetGlobalSetpointCallback callback)
{
...
void OffboardServerImpl::process_position_target_global_int(const mavlink_message_t& message) {
    ...
    //position.altitude_type = (OffboardServer::PositionGlobalYaw::AltitudeType)global_position_message.coordinate_frame;
    position.alt_m = global_position_message.alt;
    position.yaw_deg = global_position_message.yaw;

    if(_position_target_callback != nullptr) {
